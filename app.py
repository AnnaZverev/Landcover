...
def get_region_info(region_name):
    ...
    regions = {
        # --- Критерий 1: Лидеры агросектора ---
        "Краснодарский край (Тихорецк)": {
            "center": [45.85, 40.12],
            "bbox": [39.92, 45.75, 40.32, 45.95]
        },
        "Ростовская область (Сальск)": {
            "center": [46.47, 41.53],
            "bbox": [41.33, 46.37, 41.73, 46.57]
        },

        # --- Критерий 2: Указанные вами регионы ---
        "Волгоградская область (Палласовка)": {
            "center": [50.04, 46.88],
            "bbox": [46.68, 49.94, 47.08, 50.14]
        },
        "Средний Урал (окр. Екатеринбурга)": {
            "center": [56.85, 61.10], # Смещено на восток от города
            "bbox": [60.90, 56.75, 61.30, 56.95]
        },

        # --- Критерий 3: Географический охват (регионы, подверженные опустыниванию) ---
        "Средняя полоса (окр. Тулы)": {
            "center": [54.20, 37.61],
            "bbox": [37.41, 54.10, 37.81, 54.30]
        },
        "Дальний Восток (Амурская область)": {
            "center": [50.25, 127.53], # Благовещенск
            "bbox": [127.33, 50.15, 127.73, 50.35]
        },
         "Калмыкия (Элиста)": { # Оставляем как ключевой регион
            "center": [46.307, 44.258],
            "bbox": [44.058, 46.207, 44.458, 46.407]
        }
    }
    ...
    with gr.Row():
        with gr.Column(scale=1):
            region_dropdown = gr.Dropdown(
                ...
                value="Калмыкия (Элиста)" # Значение по умолчанию
            )
